...
export class P2PClient {
  ...
  private connectedPeersSet = new Set<string>();
  ...
  public initConnection(
    ...
    // 3. Continuous Standby Heartbeat (Every 4 seconds)
    this.heartbeatTimer = setInterval(() => {
      this.sendHeartbeat();
    }, 4000);

    // 4. Prune Stale Disconnected Peers (Check every 5 seconds)
  ...
  private notifyPeerConnected(peerId: string, deviceName: string) {
    const isFirstTime = !this.connectedPeersSet.has(peerId);
    this.connectedPeersSet.add(peerId);

    this.onStatusCallback?.(peerId, 'CONNECTED', deviceName);

    // Only fire initial sync on first new connection, not recurring heartbeats
    if (isFirstTime) {
  ...
  private notifyPeerDisconnected(peerId: string) {
    if (this.connectedPeersSet.has(peerId)) {
      this.connectedPeersSet.delete(peerId);
  ...
  private setupConnectionListeners(peerId: string, deviceName: string, pc: RTCPeerConnection) {
    ...
    pc.onconnectionstatechange = () => {
      if (pc.connectionState === 'connected') {
        this.notifyPeerConnected(peerId, deviceName);
